...
import { useCompanyDashboard, useAccessibleCompanies } from '@/hooks/use-dashboard'
...
export function ExecutiveDashboard() {
  ...
  if (companyQuery.isError || !companyQuery.data) return (
    ...
    <div className="space-y-5">
      ...
      {data.topAbsentDepartments.length > 0 && (
        <div className="rounded-2xl border border-border bg-background p-4 shadow-sm">
          <p className="mb-3 text-sm font-semibold text-foreground">แผนกที่ขาดงานสูงสุด</p>
          <ul className="space-y-2">
            {data.topAbsentDepartments.map((d, i) => (
              <li key={d.departmentName} className="flex items-center justify-between text-sm">
                ...
                <span className="font-bold text-red-600">{d.absentCount} คน</span>
              </li>
            ))}
          </ul>
        </div>
