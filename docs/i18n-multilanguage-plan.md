...
> **สถานะเอกสาร:** ✅ อนุมัติแล้ว (D1–D7 ครบ 2026-09-16) · **Phase 0 เสร็จ 2026-09-14** · **Phase M งานครบ + แก้บั๊กชั้น controller แล้ว 2026-09-14** (ยังไม่ commit · `migration-v1-1-2.sql` รันบน dev แล้ว **ยังไม่ deploy prod**) · **Phase 1 งานโค้ดครบ 14/14 2026-09-14** (รอตรวจรับบน LINE จริง) · **Phase 2 เสร็จ 8/10** · **Phase 3 เสร็จ 9/11 (2026-09-16)** — flow ที่เปิดใช้จริง (ticket, memo) ครบทั้ง error และ validator แล้ว · วัดด้วย `pnpm i18n:scan-api` ได้แล้ว · เหลือ 3.5 (ลา/OT/ลงเวลา) กับ 3.8 (เบิกค่าใช้จ่าย) ที่ยังไม่เปิดใช้จริง · **Phase 4 เสร็จ 17.5/18 (2026-09-16)** — D8–D11 เคาะครบ, โค้ด+migration พร้อม deploy, **commit แล้ว** (`0b1a45c`, `7c1ffdb`) · ค้างแค่ยิง push จริงบน LINE (โควตาเต็ม 300/300 ไม่เกี่ยวกับงานนี้ ดู [`line-push-quota-plan.md`](line-push-quota-plan.md)) → รายละเอียดที่ [`notification-i18n-plan.md`](notification-i18n-plan.md)
...
| 4 | LINE notification | 18 | 17.5 | 🟡 **N0–N4 เสร็จ, commit แล้ว** → [`notification-i18n-plan.md`](notification-i18n-plan.md) · เหลือยิง push จริงบน LINE รอโควตา (ดู [`line-push-quota-plan.md`](line-push-quota-plan.md)) | ~~2–3~~ **4–5 วัน** |
| 5 | ภาษาอินโดนีเซีย | 4 | 0 | ⬜ ยังไม่เริ่ม | 1–2 วัน |
| | **รวม** | **73** | **64.5** | | **16–24 วัน** |
...
### 🟡 Phase 4 — LINE notification · 17.5/18

> เฟสนี้แตะ DB และ job — แยกออกมาเพื่อให้ 3 เฟสแรกปล่อยได้ก่อนโดยไม่ต้องรอ
>
> 📄 **แผนละเอียด + ตัว checkbox จริงอยู่ที่ [`docs/notification-i18n-plan.md`](notification-i18n-plan.md)** — สำรวจโค้ดจริงแล้วพบว่างานย่อยที่ร่างไว้ตอนแรกนับแค่งานแปล **ยังไม่ได้นับงานรื้อโครง** ของจริงแตกเป็น **18 งานย่อย 5 เฟส (N0–N4) / 4–5 วัน** ครอบอีก 2 ก้อนที่แผนนี้ตกไปด้วย (การ์ดเลือกสีจากการอ่านคำไทย · LINE webhook 7 ไฟล์) · **D8–D11 เคาะครบ 2026-09-16** · 3 บรรทัดข้างล่างเป็นสรุปเดิมของเอกสารนี้ ไม่ใช่ที่ติ๊กจริง — ไปติ๊กที่แผนละเอียด

**สรุปผล (รายละเอียดที่ N0–N4 ของแผนละเอียด):**

- [x] **4.1 (ขยายเป็น N2.1)** `PreferredLanguage` เพิ่มทั้ง `Employee` **และ** `ExternalReporter` (D11 เคาะว่าต้องมีทั้งคู่ ไม่ใช่แค่ `Employee`) — ไม่มี `PATCH /me/preferences` ตามที่ร่างไว้เดิม เปลี่ยนเป็น header `X-Locale` ที่ frontend แนบทุก request แล้ว middleware เขียน DB แบบ lazy (D9)
- [x] **4.2 (= N1)** `PayloadJson` เป็น `{ templateKey, params }` แล้ว render ตอนส่งตามภาษาผู้รับ · อ่าน payload รูปเก่าได้ (ตรึงด้วยเทสต์ใน N4.2 เพราะยิงจริงไม่ได้ช่วงโควตาเต็ม)
- [x] **4.3 (ขยายเป็น N0 + N2 + N3)** ทำ N0 ก่อนตามที่เตือนไว้ — `ResolveTicketStyle` เปลี่ยนไปอ่าน `EventType` แล้ว · ครอบทั้ง 2 ก้อนที่เคยตกไป: การ์ดที่เหลือใน `LineFlexBuilder` ทั้งหมดแปลแล้ว (`BuildOtpCard` เจอว่าเป็น dead code เลยลบทิ้งแทน) · webhook 7 ไฟล์แปลแล้ว ยกเว้น 7 คำสั่งปุ่ม rich menu ที่ตั้งใจคงไทยไว้ (ดู N3.3)

> ~~4.4 master data~~ → ย้ายไปทำก่อนแล้วใน **Phase M** ตามที่เคาะ D4

**ตรวจรับ Phase 4**

- [x] เคลียร์คิว notification ก่อน deploy + ทดสอบ payload รูปแบบเก่ายังส่งออกได้ — SQL เคลียร์คิวพร้อมใช้ (N4.2) · payload เก่าตรึงด้วยเทสต์เพราะยิงจริงไม่ได้ (โควตาเต็ม)
- [ ] ผู้ใช้ที่ตั้งภาษา en ได้รับการ์ด LINE เป็นอังกฤษ, ที่ตั้ง th ยังได้ไทยเหมือนเดิม — **ตรวจได้เฉพาะฝั่ง reply แล้ว** (เมนู/ลงเวลา/เช็คอิน-เอาต์/ตรวจสอบสิทธิ์ ไม่กินโควตา) ฝั่ง push (ticket/memo/ใบลา) รอโควตารีเซ็ตหรือ LINE OA ทดสอบ
- [x] `pnpm test:api` ผ่าน — เทียบเท่า `dotnet test Hrms.slnx` **441 pass / 1 skip**
...
|---|---|---|---|---|---|
| 2026-09-16 | **Phase 4 เสร็จ 17.5/18** | N0–N4 ของ [`notification-i18n-plan.md`](notification-i18n-plan.md) ครบตามที่ร่างไว้ด้านล่าง — สรุปย่อไว้ที่หัวข้อ Phase 4 ด้านบน รายละเอียดทั้งหมด (D8–D11, งานย่อยรายจุด, บั๊กที่เจอ, ผลตรวจ) อยู่ในแผนละเอียด **ไม่ซ้ำที่นี่** | `0b1a45c`, `7c1ffdb` | | เหลือ **N4.3b** ยิง push จริงบน LINE — โควตา 300/300 เต็มตั้งแต่ก่อนเริ่มตรวจรับ ไม่เกี่ยวกับงานนี้ (ดู [`line-push-quota-plan.md`](line-push-quota-plan.md)) · ฝั่ง reply ตรวจผ่านแล้วเพราะไม่กินโควตา |
